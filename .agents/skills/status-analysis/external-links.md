...
**Note**: For `/update-weekly-status`, PR data is pre-gathered by the Python data gatherer script (`gather_status_data.py`) using GitHub's GraphQL API. This module is primarily used by `/status-rollup` which fetches PR data on-demand via the `gh` CLI.

**Future improvement**: This module could be combined with the `/extract-prs` skill to share PR extraction logic.
