...
This skill provides the core analysis logic shared by status-related commands (`/status-rollup` and `/update-weekly-status`). It handles data collection, activity analysis, and status generation in a unified way.
...
- `/status-rollup` - Single root issue, outputs as Jira comment
- `/update-weekly-status` - Multiple root issues (batch), outputs to Status Summary field
