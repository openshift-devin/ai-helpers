...
|--------|---------|---------------|--------|
| `wiki_comment` | `/status-rollup` | Jira comment | Jira wiki markup |
| `ryg_field` | `/update-weekly-status` | Status Summary field | Bullet-point template |

## Format: wiki_comment

Used by `/status-rollup` to post a comprehensive status comment to a Jira issue.
...
Used by `/update-weekly-status` to update the Status Summary custom field.
