...
if [ -f ~/.config/claude-code/olm-agent-config.json ]; then
  ...
  echo "Configuration file not found. Please run /configure-agent to create it."
  exit 1
fi
```
...
for path in $(jq -r '.. | strings | select(startswith("/"))' ~/.config/claude-code/olm-agent-config.json); do
  if [ ! -d "$path" ]; then
    ...
    echo "Run /dev-setup to clone missing repositories"
  fi
done
```
...
Run /configure-agent to update paths or /dev-setup to clone missing repositories.
```
