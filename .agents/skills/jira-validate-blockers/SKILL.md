...
command: /validate-blockers
---

# JIRA Release Blocker Validator - Implementation Guide

This skill provides detailed implementation guidance for the `/validate-blockers` command, which helps release managers make data-driven blocker approval/rejection decisions.

## When to Use This Skill

This skill is invoked automatically when the `/validate-blockers` command is executed. It provides step-by-step implementation details for:
...
**Validate inputs:**
- If neither `--bug` nor `target-version` is provided, error out with message: "Error: Either target-version or --bug must be provided. Usage: /validate-blockers [target-version] [component-filter] [--bug issue-key]"
