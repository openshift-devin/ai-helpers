...
This skill is automatically invoked by the `/create epic` command to guide the epic creation process.
...
**See:** `/create` command documentation for complete parent linking hierarchy and implementation strategy.
...
**Input:**
```bash
/create epic CNTRLPLANE "Multi-cluster metrics aggregation" --parent CNTRLPLANE-100
```
...
**Input:**
```bash
/create epic CNTRLPLANE "Advanced node pool autoscaling for ARO HCP"
```
...
**Input:**
```bash
/create epic MYPROJECT "Improve test coverage for API endpoints"
```
