...
This skill provides detailed step-by-step implementation guidance for the `/generate-feature-doc` command, which generates comprehensive feature documentation by recursively analyzing a Jira feature and all its related issues and GitHub pull requests.
...
This skill is automatically invoked by the `/generate-feature-doc` command and should not be called directly by users.
...
**Important**: This step expects PR data as input from the `extract-prs` skill (invoked by the command file). The input is structured JSON containing all discovered PRs with their metadata.
...
1. **Write documentation**: Save to `${WORK_DIR}/feature-doc.md` with footer:
   ```markdown
   ---
   *Generated by `/generate-feature-doc` on <timestamp>*
...
```
User runs: /generate-feature-doc OCPSTRAT-1612
