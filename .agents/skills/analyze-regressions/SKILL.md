...
3. **Required Scripts**
   ...
   - `../get-release-dates/get_release_dates.py`
   - `../list-regressions/list_regressions.py`
   - `./generate_html_report.py` (for HTML reports)
   - `./report_template.html` (for HTML reports)
...
```
/analyze-regressions 4.17
/analyze-regressions 4.21 --components Monitoring etcd
```
...
```bash
python3 ../get-release-dates/get_release_dates.py \
  --release 4.17
```
...
```bash
python3 ../list-regressions/list_regressions.py \
  --release 4.17 \
  --start 2024-05-17 \
  --end 2024-10-29 \
  --short
...
6. **HTML Template Not Found**
   ...
   - **Solution**: Verify template exists at `./report_template.html`
...
```
/analyze-regressions 4.21 --components "kube-apiserver"
```
...
```
/analyze-regressions 4.17 --compare 4.16
```
...
```
/analyze-regressions 4.17 --export-csv
```
...
```
/analyze-regressions 4.17 --triage-threshold 80
```
...
- `/analyze-regressions` command (primary)
...
- `analyze-prow-job-test-failure` - Analyzes individual test failures
