...
This skill is automatically invoked by the `/create task` command to guide the task creation process.
...
**Input:**
```bash
/create task CNTRLPLANE "Update autoscaling documentation for 4.21 release"
```
...
**Input:**
```bash
/create task MYPROJECT "Refactor validation logic to reduce duplication"
```
...
**Input:**
```bash
/create task CNTRLPLANE "Add integration tests for node autoscaling" --parent CNTRLPLANE-100
```
...
**Input:**
```bash
/create task CNTRLPLANE "Investigate intermittent timeouts in etcd health checks"
```
