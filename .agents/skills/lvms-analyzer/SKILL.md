...
This skill is automatically invoked by the `/analyze` command when working with must-gather data.
...
```bash
python3 ./scripts/analyze_lvms.py \
    {must-gather-path}
```

**Script Location:**
- Always use: `./scripts/analyze_lvms.py`
...
ls ./scripts/analyze_lvms.py

# Ensure it's executable
chmod +x ./scripts/analyze_lvms.py
```
