...
```bash
python3 ./list_components.py
```

**List OCPBUGS components for a specific team:**
```bash
python3 ./list_components.py --team "API Server"
```
...
```bash
python3 ./list_components.py | jq '.total_components'
```
...
```bash
python3 ./list_components.py | jq '.components[] | select(contains("apiserver"))'
```
...
- Related Skill: `../list-teams/SKILL.md`
- Related Command: `/list-components`
