...
- Another command needs to run golangci-lint as a prerequisite step (e.g., `lint-fix`)
...
- Remember the exact `golangci-lint` command that was used, as callers (like `lint-fix`) may need it
- If the user passes additional flags, chain them to the `golangci-lint` invocation (e.g., `--tests`, `--concurrency 4`)
- Do not run with `--fix`; if the user wants fixes, direct them to the `/lint-fix` command
