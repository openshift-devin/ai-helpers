...
```
@skills:analyze-regression <regression id>
```

## Description

The `analyze-regression` skill analyzes details for a specific Component Readiness regression and suggests next steps for investigation.

The skill performs a full analysis regardless of whether the regression has been triaged. For triaged regressions, it also fetches the linked JIRA issue to analyze whether someone is actively working on the fix or if the issue needs attention.
...
3. **Fetch Regression Details**: Use the `fetch-regression-details` skill
   ...
   ```bash
   script_path="../fetch-regression-details/fetch_regression_details.py"
   ...
   **Converting `test_details_url` to UI URL**: The `test_details_url` from the API is an API endpoint not suitable for display or bug reports. Convert it to the UI URL by replacing the base path. The query parameters are identical:
   ...
   See `../fetch-regression-details/SKILL.md` for complete implementation details.
...
4. **Fetch Global Test Report**: Use the `fetch-test-report` skill to check how this test is doing globally
   ...
   script_path="../fetch-test-report/fetch_test_report.py"
...
5. **Check Triage Status and Fetch JIRA Progress**: Determine triage state and analyze bug progress
   ...
   **If `triage_count > 0` (regression is triaged)**:
   ...
   if [ -z "$JIRA_TOKEN" ]; then
     ...
     jira_script="../fetch-jira-issue/fetch_jira_issue.py"
   ...
   See `../fetch-jira-issue/SKILL.md` for complete implementation details.
   ...
   **Note for Failed Fixes (analysis_status == -1000)**:
   ...
   - Flag: "FAILED FIX: Test continues to fail after triage was resolved"
...
7. **Analyze Failure Output Consistency**: Use the `fetch-test-runs` skill
   ...
   **Implementation**:
   ...
   script_path="../fetch-test-runs/fetch_test_runs.py"
   ...
   See `../fetch-test-runs/SKILL.md` for complete implementation details.
...
9. **Identify Suspect PRs in Payload**: Use `fetch-prowjob-json` and `fetch-new-prs-in-payload` skills
   ...
   **Step 9a: Get the payload tag from the first failing run**
   ...
   # See ../fetch-prowjob-json/SKILL.md for URL conversion details
   #
   ...
   **Step 9b: Fetch new PRs in the payload**
   ...
   ```bash
   script_path="../fetch-new-prs-in-payload/fetch_new_prs_in_payload.py"
   pr_data=$(python3 "$script_path" "$payload_tag" --format json)
   ```

   See `../fetch-new-prs-in-payload/SKILL.md` for complete implementation details.
...
10. **Identify Related Regressions**: Use the `list-regressions` skill to find similar failing tests
   ...
   **Analyze Related Regressions**:
   ...
   related_data=$(python3 "../fetch-regression-details/fetch_regression_details.py" <related_regression_id> --format json)
...
11. **Find Related Triages and Untriaged Regressions**: Use the `fetch-related-triages` skill
   ...
   ```bash
   script_path="../fetch-related-triages/fetch_related_triages.py"
   related=$(python3 "$script_path" <regression_id> --format json)
   ```

   See `../fetch-related-triages/SKILL.md` for complete implementation details.
...
- For high-level component health analysis, use `health-check-regressions` instead
- For listing all regressions, use `list-regressions`
- For questions, ask in #forum-ocp-release-oversight

## See Also

- Related Skill: `fetch-regression-details` - Fetches regression data with pass sequences (`../fetch-regression-details/SKILL.md`)
- Related Skill: `fetch-test-report` - Fetches global test health report with per-variant breakdown and open bugs (`../fetch-test-report/SKILL.md`)
- Related Skill: `fetch-releases` - Determines the latest OCP release (`../fetch-releases/SKILL.md`)
- Related Skill: `fetch-test-runs` - Fetches and analyzes test failure outputs (`../fetch-test-runs/SKILL.md`)
- Related Skill: `fetch-prowjob-json` - Fetches prowjob.json for payload tag and metadata (`../fetch-prowjob-json/SKILL.md`)
- Related Skill: `fetch-new-prs-in-payload` - Fetches new PRs in a payload (`../fetch-new-prs-in-payload/SKILL.md`)
