...
- Repository checkout so the scripts under `./` are accessible.
...
2. **Inspect or Label Nodes (optional)**
   ...
   python3 ./generate_tuned_profile.py --list-nodes --node-selector "node-role.kubernetes.io/worker" --skip-manifest

   # Label a specific node for the worker-hp pool
   python3 ./generate_tuned_profile.py \
...
- **Live node analysis**
  ```bash
  python3 ./analyze_node_tuning.py --format markdown
  ```
- **Remote analysis via oc debug**
  ```bash
  python3 ./analyze_node_tuning.py \
    --node worker-rt-0 \
...
4. **Act on recommendations**
   ...
   - Feed actionable items back into `/generate-tuned-profile` to codify desired state.
