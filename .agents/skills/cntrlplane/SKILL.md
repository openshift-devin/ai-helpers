...
This skill is automatically invoked by the `/create` command when the project_key is "CNTRLPLANE".
...
**Note:** Universal requirements (Security Level: Red Hat Employee, Labels: ai-generated-jira) are defined in the `/create` command and automatically applied to all tickets.
...
**See:** `/create` command documentation for the complete "Issue Hierarchy and Parent Linking" reference, including field mapping, MCP code examples, and fallback strategies.
...
Follow the implementation strategy documented in `/create` command:
...
**Note:** Security validation (credential scanning) is defined in the `/create` command and automatically applied to all tickets.
...
**Note:** All examples automatically apply universal requirements (Security: Red Hat Employee, Labels: ai-generated-jira) as defined in `/create` command.

### Create CNTRLPLANE Story

```bash
/create story CNTRLPLANE "Enable pod disruption budgets for control plane"
```
...
```bash
/create epic CNTRLPLANE "Improve cluster lifecycle management"
```
...
```bash
/create feature CNTRLPLANE "Advanced observability capabilities"
```
...
```bash
/create task CNTRLPLANE "Refactor cluster controller reconciliation logic"
```
...
**Note:** Jira description formatting (Wiki markup) is defined in the `/create` command.
...
When `/create` is invoked for CNTRLPLANE:
...
**Note:** Universal requirements (security, labels), security validation, and issue creation handled by `/create` command.
...
**Note:** Universal best practices (security, labels, formatting, credential scanning) are defined in the `/create` command.

## See Also

- `/create` - Main command that invokes this skill (includes Issue Hierarchy and Parent Linking documentation)
