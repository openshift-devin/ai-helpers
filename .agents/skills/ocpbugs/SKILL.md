...
This skill is automatically invoked by the `/create` command when the project_key is "OCPBUGS" or when issue type is "bug" without a project specified.
...
```bash
/create bug "Control plane pods crash on upgrade from 4.20 to 4.21"
```
...
```bash
/create bug OCPBUGS "API server returns 500 error when creating namespaces" --component "API" --version "4.21"
```
...
**Note:** Universal best practices (security, credential sanitization, formatting) are defined in the `/create` command.

## See Also

- `/create` - Main command that invokes this skill
